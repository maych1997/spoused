...
import {addChatMessage, updatedMessages, addUpdatedMessages} from "../../../redux/authSlice";
...
const ChatScreen = (props: any) => {
    ...
    const chatdata = async () => {
        try {
            const res = await singleChatApi(route?.params?.conversation, reduxState.auth.token);

            // console.log("chat data", res);
            const chatSeener = await MarkReadChats(route?.params?.conversation, reduxState.auth.token);
    ...
    useEffect(() => {
        ...
        const imageHelper = imageUser.find(item => item.id === route.params.user._id);
    ...
    useEffect(() => {
        ...
        useEffect(() => {
        socket.current = io("https://api.getspoused.com", {
            path: '/socket',
            auth: {
        ...
        if (arrivalMessage) {
            ...
            const object = {
                lastMessage: {
                    ...
                    content: arrivalMessage.type === "audio"
                        ? "Voice Message"
    ...
    const getMessages = async () => {
        ...
        try {
            const res = await getChatApi(route?.params?.conversation, reduxState.auth.token);
            ...
            const object = {
                lastMessage: {
                    ...
                    _id: "671cf00635356fdc22b2c1fc",
                    chat: currentChat,
                    content: await updatedMessages2[updatedMessages2.length - 1]?.content || "message",
                    createdAt: new Date().toISOString(),
                    sender: await updatedMessages2[updatedMessages2.length - 1]?.sender || reduxState?.auth?.user?.id,
                    type: "text",
                    updatedAt: new Date().toISOString(),
                },
                numberOfUnseen: 0,
    ...
    const phoneRegex = /(\b\d{10,}\b)|(\+?\d{1,3}[-.\s]?\(?\d{1,4}\)?[-.\s]?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9})/g;
    ...
    const sendMessage = async () => {
        ...
        if (containsPhoneNumber || containsEmail) {
            ...
            openModal("It is forbidden to send phone numbers including email addresses", "In case you are trying, it’s likely you’ll be banned", "OK", "error");
    ...
    const sendCall = async () => {
        if (reduxState?.auth?.user?.myprofile?.proAccount){
            ...
            try {
                const res = await sendCallApi(route?.params?.conversation, roomId, reduxState.auth.token);
            ...
            openModal("Upgrade to Pro Plan", "To make voice calls, you need to upgrade to a Pro Plan", "OK", "error");
    ...
    const Block = async () => {
        try {
            const res = await blockUserApi(route?.params?.user?._id, reduxState.auth.token);
    ...
    const handleUnmatch = async () => {
        try {
            const res = await unmatchUserApi(route?.params?.user?._id, reduxState.auth.token);
    ...
    const renderItem = (item: any, index: any) => {
        ...
        const renderMessageContent = () => {
            switch (item.type) {
                case "image":
                    const source = item.typebase === "base64"
                    ...
                    return (
                        <Image
                            source={source}
                            style={Platform.OS === "ios" ? {width: 250, height: 250} : {width: 200, height: 200}}
                        />
                    );
                ...
                case "text":
                    return (
                        <View>
                            <Text style={[styles.messageText, {fontFamily: "Poppins_400Regular"}]}>
                                {item.content}
                            </Text>
                ...
                case "audio":
                    ...
                    return (
                        <View style={{
                            ...
                            <TouchableOpacity
                                onPress={() => isPlaying2 && currentAudio === item._id ? pause() : play(audioContent, item._id)}
                                style={{
                                    height: 35,
                                    width: 35,
                            ...
                            <Slider
                                ...
                                value={currentAudio === item._id ? playbackStatus2?.positionMillis || 0 : 0}
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            <View style={isCurrentUser ? {
                                ...
                                <FontAwesome name={item.content.includes("voice") ? "phone" : "video-camera"} size={19}
                                             color="black"/>
                            </View>
                            <View>
                                <Text style={[styles.messageText, {fontFamily: "Poppins_400Regular"}]}>
                                    {isCurrentUser ? "You" : route.params?.user?.fullName} started
                                    a{item.content.includes("voice") ? "" : " video"} call
                                </Text>
                                <Text style={[styles.messageText, {fontFamily: "Poppins_300Light", fontSize: 12}]}>
                                    Click to join
                                </Text>
                            </View>
                        </TouchableOpacity>
                          ...
                          >
                            ...
                            <View>
                              ...
                              >
                                ...
                                {item.content.includes("voice") ? " a Call" : " a video Call"}
                              </Text>
                              <Text
                                style={[
                                  styles.messageText,
    ...
    const handleRematch = async () => {
        ...
        try {
            const res = await rematchUserApi(route?.params?.user?._id, reduxState.auth.token);
    ...
    if (videoCall)
        return (
            <AgoraUIKit connectionData={videoConnectionData} rtcCallbacks={callbacks}/>

        )
    if (voiceCall)
        return (
            <AudioCallUiKit username={route?.params?.user.fullName} userImage={image}
                            appId={voiceConnectionData.appId} callState={voiceCall}
                            channelName={voiceConnectionData.channel} onHangUp={onHangUp} token=""/>
        )
    else
        return (
                ...
                >
                    <FlatList
                        ...
                        onScrollToIndexFailed={(error) => {
                            listViewRef.current?.scrollToOffset({ offset: error.averageItemLength * error.index, animated: true });
                        }}
                    />
                    <ChatSender
