...
const ChatOverview = (props: any) => {
  ...
  useEffect(() => {
    ...
    getLastChatApi(props?.user?.users?._id, token)
      .then((fetchData) => {
        if (fetchData.message != "Error fetching chat details") {
          let checkData = fetchData;
          (checkData.lastMessage.content =
            checkData?.lastMessage?.type === "audio"
              ? "Voice Message"
  ...
  useEffect(() => {
    const imageHelper = imageUser?.find(
      (item) => item.id === props.user.users._id
    );
    ...
    if (chatData.length > 0) {
      ...
      const latestChat = chatData.find(
        (item) => item.lastMessage.chat === chatUniqId
      );
      ...
      const time = new Date(
        latestChat?.lastMessage?.createdAt
      ).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
        hour12: true,
      ...
      setLastMessage("Write a Message");
    }
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1, marginHorizontal: 10 }}>
        ...
        >
          {lastMessage==undefined?'Write a message':lastMessage}
        </Text>
      </View>
