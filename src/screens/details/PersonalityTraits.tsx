...
import { updatePersonalityApi } from "../../../api/ProfileCompletion/PutApis/updateProfileApi";
import { updatemyprofile, updatetraitsfilter } from "../../../redux/authSlice";
...
const PersonalityTraits = (props: any) => {
  ...
  const { back, setTraitsFilter, traitsFilter } = route.params;
  const [selectedTraits, setSelectedTraits] = useState(traitsFilter || []);
  ...
  useEffect(() => {
    if (back === 0 && reduxState?.auth?.user?.myprofile?.personalityTraits) {
      setSelectedTraits(reduxState?.auth?.user?.myprofile?.personalityTraits);
    }
  }, []);

  const updateDetails = async () => {
    try {
      const response = await updatePersonalityApi(
        selectedTraits,
  ...
  const handleDone = () => {
    if (back === 0) {
      ...
      if (reduxState?.auth?.user?.myprofile?.proAccount) {
        console.log("this is the personality traits from advance filters");

        setTraitsFilter(selectedTraits);
        props.navigation.navigate("Tab", { screen: "Filters" });
  ...
  return (
    <SafeAreaView style={[styles.container, globalStyles.androidSafeArea]}>
      <ScrollView style={{ paddingTop: 15 }}>
        ...
        <Personality
          selectedTraits={selectedTraits || []} // Provide an initial value for selectedTraits
